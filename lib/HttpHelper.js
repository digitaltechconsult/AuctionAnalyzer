...
HttpHelper.prototype.get = function (url, onSuccess, onFail, onProgress) {
    ...
    //make GET request
    var httpRequest = $this.httpObject.get(url, function (response) {
        var responseString = '';
    ...
    //handle errors if network doesn't work or address is incorrect
    httpRequest.on('error', function(error) {
        console.error("Please check network connection");
        $this.eventEmmiter.emit($this.event.fail, error);
    });
}
